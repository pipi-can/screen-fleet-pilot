...
ScheduleMgr::ScheduleMgr() : m_timerFd(-1), m_nextId(1) {}
...
time_t ScheduleMgr::parseTriggerAt(const std::string& scheduleDate, const std::string& scheduleTime) {
...
void ScheduleMgr::reloadFromDb() {
    ...
    m_taskMap.clear();
    m_nextId = 1;
    std::vector<ScheduleTask> tasks = DatabaseMgr::getInstance().queryPendingScheduleTasks(time(nullptr));
    int maxId = 0;
    ...
    if (maxId > 0) m_nextId = maxId + 1;
}

void ScheduleMgr::pushHeap(const ScheduleTask& task) {
    if (task.enabled) m_heap.push(task);
}
...
void ScheduleMgr::updateTimerfd() {
    ...
    timerfd_settime(m_timerFd, TFD_TIMER_ABSTIME, &its, nullptr);
}

void ScheduleMgr::executeTask(const ScheduleTask& task) {
    if (!task.enabled || task.paths.empty()) return;
    bool ok = PushMgr::getInstance().pushResourcesToEmbedded(task.deviceUid, task.paths);
    if (!ok) {
        logger->logMsg(WARNING, "schedule execute failed id=" + std::to_string(task.id), true);
    } else {
        logger->logMsg(DEBUG, "schedule execute ok id=" + std::to_string(task.id), true);
    }
...
std::vector<std::string> ScheduleMgr::parsePaths(const char* pathsJson) {
    ...
    if (!pathsJson || pathsJson[0] == '\0') return paths;
