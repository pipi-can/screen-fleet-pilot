...
def simulate_device(idx: int, host: str, port: int,
    ...
    while not STOP_FLAG:
        try:
            ...
            # ── 发送注册 ──
            SEQS[tid] += 1
            send_line(sock, make_msg("device", "register", SEQS[tid], {
                "name": name,
                "group": group,
                "version": version,
            }))
            ...
            while not STOP_FLAG:
                ...
                SEQS[tid] += 1
                send_line(sock, make_msg("device", "status", SEQS[tid], {
                    "temperature":  f"{temp}°C",
                    "mem_usage":    mem,
                    "disk_free_mb": disk,
                }))

                # 检查是否有来自 server 的下发指令
                line = recv_line(sock, timeout=0.3)
                if line:
                    try:
                        msg = json.loads(line)
                        cmd = msg.get("cmd", "")
